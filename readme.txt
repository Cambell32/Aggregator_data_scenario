...
10/08/2025 
Data extracted from SQL, requires transformation in Python before being imported into Power BI for visualisation. 
...
18/08/2025
Currently working on extracting data from Aggre db via python, in file aggre_db_select.py. At the moment just mucking around with formatting loan application dates 
ahead of Power BI visualisation.  

21/08/2025
Working on returning a query result from aggre_db_select.py and then cleaning / formatting data in the clean_data.py of the aggre_db_python folder. 
(This division is just to ensure one file per function). 
At the moment, trying to find a work around with the SQL result and dataframe. Refer to the file for more the precise issue. 

24/08/2025
Currently working on a clean_data module where an SQL result from the select() is checked, cleaned-up, and then exported as a csv file. 

Trying to find the right balance of checking, cleaning, and formatting in attempt to emulate a real world situation where the data
may not be the best because of the data pipeline. 


25/08/2025
Able to create a file and instantiate it with the desired filepath but unfortunately the file is not 
generated as a CSV file. Need to figure out why. 


31/08/2025
Consolidated all of the CSV files into a single excel book. Reason being, Power BI is 'intelligent' enough to make relationship connections
between tables. Significantly easier than setting-up the matching rows manually within  Power BI. 

My logic here is that, on any given job, I am going to do precisely the function that leads to the output that is precisely required 
with no more 'fancy' foot work than is required. 

Imported and transformed aggre_db CSV files. 

Power BI: I need to run a DAX on Marketing table Marketing Name stripping the excess values from the row based upon the Marketing Channel. 
7 = Google SEO 
1 = Facebook Offer by Lender
2 = Jobboard 

In those rows, to ensure the marketing ID is still valid and can be joined to the rows in the loan_application table, there
can only be 1 marketing ID per Marketing Name. 

Potential matching error on aggre_db_organization table:the first row has been as 2, meaning that if loan_application contains rows
with an organisation ID of 1, could mean that a lot of the organization data (for lenders or brokers) for the value of 1 in that table
may need to be handled. 

Handling the loan_applications and organization tables. 
Has the org table contains both lenders and brokers it is tricky to match based on the organization_id column to the corresponding 
loan_application broker and lender columns of that table. 


06/09/2025
Sunk serious time into generating new data for Power BI analysis as there was a critical failure between tables loan applications and clients: nonfirsthome buyer client rows 22384 - 79899 were missing. 

Regenerated in clients_table_completed but now need to reinsert into aggre_db sql into to ensure data consistency across all data. 
